"pwr.anova.test" <-
function (k = NULL, n = NULL, f = NULL,  
    sig.level = 0.05, power = NULL) 
{
    if (sum(sapply(list(k, n, f, power, 
        sig.level), is.null)) != 1) 
...
if (!is.null(k) && k < 2) 
 ...
 if (!is.null(power) && !is.numeric(power) || any(0 > 
    ...
    if (is.null(power)) 
        ...
        k <- uniroot(function(k) eval(p.body) - power, 
            c(2 + 1e-10, 100))$root
    else if (is.null(n)) 
        n <- uniroot(function(n) eval(p.body) - power, c(2+ 1e-10, 1e+05))$root
    else if (is.null(within.var)) 
        f <- uniroot(function(f) eval(p.body) - 
            power, c(1e-07, 1e+07))$root
